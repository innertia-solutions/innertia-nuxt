<script setup>
// Panel drop-in de observabilidad: sub-nav interna. Fase 1: BD (Dashboard nuevo),
// Errores/Logs (componentes existentes), Archivos (placeholder Fase 3).
const TABS = [
...
<template>
  <div class="space-y-5">
    <nav class="flex gap-1 border-b border-card-line">
      <button v-for="t in TABS" :key="t.key" type="button" @click="active = t.key"
        class="px-3 py-2 text-sm font-medium transition-colors -mb-px border-b-2"
    ...
    <div v-show="active === 'files'">
      <p class="rounded-card border border-dashed border-card-line p-8 text-center text-sm text-muted-foreground">
        Visor de archivos de log — próximamente (Fase 3).
      </p>
    </div>
  </div>
</template>
